document.addEventListener('DOMContentLoaded', () => {
    ...
    const ownerBypassButton = document.getElementById('owner-bypass-button');
    const notificationMessage = document.getElementById('notification-message');

    if (!mainContent || !footerContent || !accessControlUI || !notifyOwnerButton || !ownerBypassButton || !notificationMessage) {
    ...
    notifyOwnerButton.addEventListener('click', () => {
        notificationMessage.textContent = 'O proprietário (cmspnunca0@gmail.com) foi notificado. Por favor, aguarde a liberação do acesso. Se você é o proprietário, pode usar o botão "Liberar Acesso Neste Navegador" abaixo para acesso imediato neste dispositivo.';
    ...
    ownerBypassButton.addEventListener('click', () => {
        localStorage.setItem('siteAccessGrantedByOwner', 'true');
        showContent();
    });
});
